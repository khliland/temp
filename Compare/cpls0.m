function [B, W, P, T, mX, cc] = cpls0(pc, X, Yprim, Ysec)
...
for j=1:pc  
   W0 = X'*Y; %[Yprim Ysec];  
   [w, cc(j)] = R(X, W0, Y(:,1:nyp)); %, wt); % Calculation of loading weights by canonical correlation
   t = X*w; t = t/norm(t); p = X'*t; q = t'*Y;
   Y = Y - t*q;  X = X - t*p';  
...
%%%%%%% Internal functions %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function [w, s] = R(X, W, Yp) 
        % XW - centred and transformed X-data matrix(independant), Yp - matrix of dependent variables, W - temporary loading weights
        [A,~,r] = ccXY(X*W, Yp); %, wt); % Computaion of canonical correlations between XW and Y with the rows weighted according to wt.
